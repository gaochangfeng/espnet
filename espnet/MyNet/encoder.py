    ...
    def forward(self, xs, masks=None):
        ...
        if self.center_len == 0:
            ...
            for i in range(len(chunks)):
                if self.abs_pos:
                    xss, maskss = self._forward(chunks[i].to(xs.device), chunks_mask[i].to(xs.device), i * self.hop_len//4)
                else:
                    xss, maskss = self._forward(chunks[i].to(xs.device), chunks_mask[i].to(xs.device), 0)
                xss = xss[:, chunk_left // 4:(chunk_left + self.center_len) // 4].cpu()
                maskss = maskss[:, :, chunk_left // 4:(chunk_left + self.center_len) // 4].cpu()
                xs_list.append(xss)
                mask_list.append(maskss)
            xs = torch.cat(xs_list, dim=1).to(cuda_id)
            masks = torch.cat(mask_list, dim=2).to(cuda_id)
            if self.use_mem:
